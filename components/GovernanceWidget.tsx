...
export function GovernanceWidget() {
  ...
  return (
    <Link href="/proposals">
      <div className={`flex items-center gap-3 px-4 py-3 rounded-lg border border-border/50 bg-card hover:bg-muted/30 transition-colors cursor-pointer border-l-4 ${borderColor}`}>
        ...
        <div className="flex items-center gap-2 min-w-0">
          ...
          <span className="text-xs text-muted-foreground">Open Proposal{totalOpen !== 1 ? 's' : ''}</span>
          {hasCritical && (
            <Badge variant="outline" className="bg-red-500/10 text-red-700 dark:text-red-400 gap-0.5 text-[10px] px-1.5 py-0">
            ...
            {totalOpen > 0 && (
              <div className="w-12 h-1.5 rounded-full bg-muted overflow-hidden">
                <div
                  ...
                  style={{ width: `${totalOpen > 0 ? (data.drepVotedCount! / totalOpen) * 100 : 0}%` }}
                />
              </div>
            )}
          </div>
