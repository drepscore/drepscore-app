...
import {
  ...
  getLevelDefinition,
  getLevelProgress,
} from '@/lib/governanceLevels';
import { posthog } from '@/lib/posthog';

const ICON_MAP: Record<string, React.FC<{ className?: string }>> = {
  Eye, Vote, Shield, Crown,
};

const COLOR_MAP: Record<string, { badge: string; icon: string; bar: string }> = {
  slate: { badge: 'bg-slate-100 text-slate-700 dark:bg-slate-800 dark:text-slate-300', icon: 'text-slate-500', bar: 'bg-slate-500' },
  blue:  { badge: 'bg-blue-100 text-blue-700 dark:bg-blue-900/40 dark:text-blue-300', icon: 'text-blue-500', bar: 'bg-blue-500' },
  amber: { badge: 'bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-300', icon: 'text-amber-500', bar: 'bg-amber-500' },
  green: { badge: 'bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-300', icon: 'text-green-500', bar: 'bg-green-500' },
};
      ...
      <CardContent className="p-4 space-y-3">
        ...
        {progress.nextLabel && (
          <div className="space-y-1">
            ...
            {progress.hint && (
              <p className="text-[10px] text-muted-foreground">{progress.hint}</p>
            )}
          </div>
        )}
