      ...
      <OGBackground glow={OG.indigo}>
        <div style={{ display: 'flex', flexDirection: 'column', width: '100%', height: '100%', padding: '64px' }}>
          <div style={{ display: 'flex', flexDirection: 'column', marginBottom: '40px' }}>
            <div style={{ display: 'flex', fontSize: '24px', color: OG.textMuted, fontWeight: 600, letterSpacing: '0.05em', textTransform: 'uppercase' as const }}>
              Epoch {epoch} Summary
            </div>
            <div style={{ display: 'flex', fontSize: '48px', color: OG.text, fontWeight: 700, marginTop: '8px' }}>
              Governance Recap
            </div>
          </div>
...
function StatBox({ label, value, color }: { label: string; value: string; color: string }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '24px 32px', borderRadius: '16px', background: OG.bgCard, border: `1px solid ${OG.border}` }}>
      <div style={{ display: 'flex', fontSize: '48px', fontWeight: 700, color }}>{value}</div>
      <div style={{ display: 'flex', fontSize: '16px', color: OG.textMuted, marginTop: '8px' }}>{label}</div>
    </div>
  );
}
