...
async function withRetry<T>(fn: () => Promise<T>, label: string, retries = 2): Promise<T> {
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      console.warn(`${TAG} ${label} attempt ${attempt + 1} failed: ${errMsg(e)}, retrying in ${delay}ms...`);
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (openProposals.length > 0) {
      try {
        ...
        console.warn(`${TAG} Voting summary refresh error:`, err instanceof Error ? err.message : err);
      }
    }
    ...
    try {
      ...
      console.warn(`${TAG} Push skipped:`, err);
    }
  } catch (err) {
  ...
  console.log(`${TAG} Complete in ${duration}s — ${proposalCount} proposals, ${voteCount} votes${pushSent > 0 ? `, ${pushSent} push` : ''}${errors.length > 0 ? ` (${errors.length} errors)` : ''}`);
