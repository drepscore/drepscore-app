#!/usr/bin/env bash
# bin/supabase-sandbox-mcp.sh
#
# Stdio wrapper that resolves SUPABASE_ACCESS_TOKEN from 1Password and execs
# the official Supabase MCP server scoped to the Governada SANDBOX preview
# branch (held open by the long-lived feat/dev-sandbox PR — see
# governada-brain/decisions/sandbox-dev-target.md).
#
# Mirrors bin/supabase-mcp.sh exactly, except the project ref comes from
# SUPABASE_SANDBOX_PROJECT_REF_OP_REF (resolved at runtime via 1Password)
...
#
# Invoked by .mcp.json under server name "supabase-sandbox". Tool calls
# become mcp__supabase-sandbox__<tool> in Claude Code; the allowlist in
# .claude/settings.json must include mcp__supabase-sandbox__*.
#
# Safety: mcp__supabase-sandbox__apply_migration is gated by the same
...
sandbox_ref_ref="$(env_or_ref_value SUPABASE_SANDBOX_PROJECT_REF_OP_REF)"
assert_op_ref SUPABASE_SANDBOX_PROJECT_REF_OP_REF "$sandbox_ref_ref"
...
op_read_secret() {
  local op_ref="$1"
  local err_file
  err_file="$(mktemp "${TMPDIR:-/tmp}/governada-supabase-sandbox-op-read.XXXXXX")" || return 1
  (
...
set +e
project_ref="$(op_read_secret "$sandbox_ref_ref")"
ref_status=$?
set -e

if [[ "$ref_status" -ne 0 || -z "$project_ref" ]]; then
  echo "Remediation: confirm SUPABASE_SANDBOX_PROJECT_REF_OP_REF resolves to the sandbox project ref per ${ADDENDUM}." >&2
  exit 1
fi
