#!/usr/bin/env bash
# bin/supabase-account-mcp.sh
#
# Stdio wrapper that resolves SUPABASE_ACCESS_TOKEN from 1Password and execs
# the official Supabase MCP server scoped to the Governada ACCOUNT (no
# --project-ref), read-only, for billing / quota / branch diagnosability.
#
# Pattern mirrors bin/supabase-mcp.sh — token resolution is identical. The
# difference is the exec line:
#   - NO --project-ref, so the account-level feature group surfaces
#     (get_cost, list_organizations, list_branches, get_advisors, ...).
#   - --read-only and --features=account,branching,debugging,docs, so the
#     database / functions / storage / development groups are dropped entirely.
#
# --read-only only wraps the database group's SQL in a read-only transaction;
# it does NOT block Management-API mutations (pause_project, create_branch,
# ...). The mutation guard for those lives in .claude/settings.json
# permissions.deny — every mutation tool this server exposes is hard-denied
# there. See governada-brain/plans/supabase-account-mcp.md.
#
# This wrapper is invoked by .mcp.json under server name "supabase-account".
# Tool calls become mcp__supabase-account__<tool> in Claude Code.
...
# exec the stdio MCP server. We deliberately do NOT pass --project-ref: the
# account feature group only surfaces when the server is account-scoped.
# --read-only plus an explicit --features allowlist shrinks the surface to
# account / branching / debugging / docs. --read-only only gates the database
# group's SQL, so the destructive Management-API tools that remain
# (pause_project, restore_project, create_project, confirm_cost, create_branch,
# delete_branch, merge_branch, reset_branch, rebase_branch) are hard-denied in
# .claude/settings.json permissions.deny. See
# governada-brain/plans/supabase-account-mcp.md.
exec env -u OP_SERVICE_ACCOUNT_TOKEN \
  ...
  npx -y @supabase/mcp-server-supabase \
    --read-only \
    --features=account,branching,debugging,docs
