...
async function main() {
  ...
  // Total counts
  const { count: totalVotes } = await sb.from('drep_votes')
    .select('*', { count: 'exact', head: true });
  const { count: withPower } = await sb.from('drep_votes')
    .select('*', { count: 'exact', head: true })
    .not('voting_power_lovelace', 'is', null);
  const { count: nullPower } = await sb.from('drep_votes')
    .select('*', { count: 'exact', head: true })
    .is('voting_power_lovelace', null);

  console.log('=== VOTE POWER COVERAGE ===');
  console.log(`Total votes: ${totalVotes}`);
  console.log(`With power:  ${withPower} (${((withPower||0)/(totalVotes||1)*100).toFixed(1)}%)`);
  console.log(`NULL power:  ${nullPower} (${((nullPower||0)/(totalVotes||1)*100).toFixed(1)}%)\n`);
  ...
  let offset = 0;
  while (true) {
    const { data } = await sb.from('drep_votes')
  ...
  const { count: nullEpoch } = await sb.from('drep_votes')
    .select('*', { count: 'exact', head: true })
    .is('voting_power_lovelace', null)
    .is('epoch_no', null);
  ...
  console.log(`NULL-power votes with valid epoch: ${[...nullVotesByDrep.values()].reduce((s, e) => s + e.length, 0)}\n`);

  // Sample up to 50 DReps (prioritize those with most NULL votes)
  const sorted = [...nullVotesByDrep.entries()]
  ...
  for (let i = 0; i < sampled.length; i++) {
    ...
    try {
      ...
      if (history.length === 0) {
        ...
        if (i < 5) console.log(`  ${drepId.slice(0, 30)}...: ${voteEpochs.length} votes, NO Koios history → Tier 3`);
      } else {
        ...
        if (i < 10) {
          console.log(`  ${drepId.slice(0, 30)}...: ${voteEpochs.length} votes, history epochs ${minHistory}-${maxHistory} (${history.length} epochs), T1=${t1} T2=${t2}`);
        }
      }
    } catch (err) {
      tier3Count += voteEpochs.length;
      if (i < 3) console.error(`  Error for ${drepId.slice(0, 30)}...: ${err instanceof Error ? err.message : err}`);
    }
  ...
  console.log(`Tier 1 (exact match):     ${tier1Count} (${(tier1Count/totalSampledVotes*100).toFixed(1)}%)`);
  console.log(`Tier 2 (nearest epoch):   ${tier2Count} (${(tier2Count/totalSampledVotes*100).toFixed(1)}%)`);
  console.log(`Tier 3 (no Koios data):   ${tier3Count} (${(tier3Count/totalSampledVotes*100).toFixed(1)}%)`);
  ...
  if (tier2Gaps.length > 0) {
    ...
    for (const [gap, count] of Object.entries(gapCounts).sort((a, b) => Number(a[0]) - Number(b[0]))) {
    ...
    console.log(`  Min gap: ${tier2Gaps[0]}, Max gap: ${tier2Gaps[tier2Gaps.length - 1]}, Median: ${tier2Gaps[Math.floor(tier2Gaps.length / 2)]}`);
  } else {
  ...
  console.log(`  Already have power: ${withPower} (${((withPower||0)/(totalVotes||1)*100).toFixed(1)}%)`);
  console.log(`  Will get exact:    ~${estT1} (${(estT1/(totalVotes||1)*100).toFixed(1)}%)`);
  console.log(`  Will get nearest:  ~${estT2} (${(estT2/(totalVotes||1)*100).toFixed(1)}%)`);
  console.log(`  Will get current:  ~${estT3} (${(estT3/(totalVotes||1)*100).toFixed(1)}%)`);
}
